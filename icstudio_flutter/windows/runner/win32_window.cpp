...
int GetResizeBorderWidth(HWND hwnd) {
  return MulDiv(kResizeBorderWidth, GetDpiForWindow(hwnd), 96);
}
...
LRESULT HitTestResizeBorder(HWND hwnd, LPARAM lparam) {
  ...
  const int border = GetResizeBorderWidth(hwnd);
...
void UpdateMaximizedBounds(HWND hwnd, LPARAM lparam) {
  ...
  info->ptMaxPosition.x = work.left - monitor_bounds.left;
  info->ptMaxPosition.y = work.top - monitor_bounds.top;
